...
const ProductRow = (props: PdtProps) => {
    ...
    return (
        ...
        <tr className='w-full h-fit flex flex-col flex-nowrap md:table-row hover:bg-cardGray-900' key={_id}>
            ...
            <td className='flex flex-row px-6 py-2 font-medium text-base whitespace-nowrap justify-between truncate peer'>
                {title}
                <div className='peer'>
                    <InfoIcon />
                </div>
                <p className="w-[200px] h-fit absolute md:right-20 xl:right-40 z-10 px-3 text-sm text-white bg-dark rounded-md shadow-sm hidden peer-hover:md:block bg-cardGray-700 truncate line-clamp-2 overflow-hidden">
                    {description}
